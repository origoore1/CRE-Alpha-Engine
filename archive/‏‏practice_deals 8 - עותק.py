...
WHAT WOULD MAKE IT BETTER:
  - Anchor concentration at 68% is above the 55% benchmark — single risk point
...
""",
    "deal": {
        ...
        "interest_rate": 0.041, "loan_term": 10,
...

{
    ...
    "learning_objective": "Understand how a small LTV breach triggers a cascade of issues.",
    "teaching_notes": """
WHAT TO LOOK FOR:
  ⚠️ LTV 65.1% — just 1.1% above the 65% bank maximum
  ⚠️ 3-year expiry risk — check when Takko and Lotto expire
  ✓ DSCR 1.37x — fine once LTV is fixed
  ✓ WAULT 5.8yr — acceptable

KEY LEARNING:
  A 1.1% LTV breach is actually easy to fix — you just need EUR 151,000 more equity.
  The Deal Repair Engine should show this precisely.
  But ask yourself: why is the LTV 65.1%? Because the asking price is slightly too high
  relative to what the bank will lend against.

  The real question here is the Takko Fashion lease expiry.
  Check the Banker Lens lease expiry chart — when does it expire?
  Takko has been contracting (fewer stores) nationally. In a 2026 market,
  renewing Takko at current rent is uncertain.

HOW TO APPROACH NEGOTIATION:
  Option A: EUR 151k more equity (easy fix if you have it)
  Option B: Negotiate 1% price reduction → EUR 73,500 → LTV drops to 64.2%
  Option C: Share deal (NRW, 6.5% GrESt) saves EUR 481,000 in tax
""",
    "deal": {
    ...
    "rent_roll": [
        ("U-01","Rewe Markt GmbH","Food & Grocery","Y",2100,11.00,
         date(2019,4,1),date(2032,3,31),None,"Y",0.75,0.10,23100,"Y",0,"Anchor 6yr"),
        ("U-02","Rossmann GmbH","Health & Beauty","N",480,15.50,
         date(2020,10,1),date(2031,9,30),None,"Y",1.00,0.10,8928,"Y",0,"5yr remaining"),
        ...
        ("U-04","Lotto/Post","Personal Services","N",440,12.50,
         date(2021,1,1),date(2028,12,31),None,"N",0.00,0.00,4950,"Y",0,"2yr remaining"),
    ],
    "costs": {
        "property_mgmt_pct":0.015,"asset_mgmt_pct":0.005,
...

{
    ...
    "teaching_notes": """
WHAT TO LOOK FOR:
  ⚠️ Two office tenants expiring in 9-12 months — 29% of income
  ⚠️ WAULT 3.5yr — office tenants dragging the average down
  ⚠️ GIY 5.04% — seller pricing as retail despite office exposure
  ⚠️ NRW GrESt 6.5% — EUR 552,500 tax on entry

KEY LEARNING:
  The seller is marketing this as 'retail' but 29% of GLA is office.
  Office market in suburban Cologne (Porz) is structurally weak post-COVID.
  When those leases expire, effective vacancy jumps from 0% to 60%+.

  The 'INVESTIGATE' verdict is actually generous. In reality, a sophisticated
  IC would require the office tenants to renew before approving the acquisition.

ADVANCED QUESTION: What does a sensitivity scenario look like?
  If both office tenants leave: NOI drops from EUR 326k to ~EUR 220k.
  New DSCR: 220k / 582k debt service = 0.38x — catastrophic.
  This deal needs a 35-40% price reduction OR a condition precedent
  requiring office renewals to be signed before closing.
""",
    "deal": {
        ...
        "asking_price": 8_500_000, "equity": 3_400_000,
        "grest_rate": 0.065, "notar_rate": 0.015, "broker_rate": 0.010,
        "analysis_date": date.today(), "hold_period": 10,
        "interest_rate": 0.041, "loan_term": 10,
        "amort_type": "Annuitaetendarlehen", "amort_rate": 0.020, "io_period": 2,
