...
def load_deal_overview(filepath) -> dict:
    ...
    overview = {
        ...
        "asset_type":            cell(7,  1),
        "gla_total":             num(8,   1),
        "market_rent_per_sqm_mo": num(10,  1),   # Asset-wide market rent fallback
...
def load_rent_roll(filepath) -> pd.DataFrame:
    ...
    rename = {
        ...
        "Notes /\nObservations":        "notes",
        "Market Rent\n\u20ac/m\xb2/mo": "market_rent_per_sqm_mo",
    }
    ...
    rename_ascii = {
        ...
        "Rent-Free\nMonths":            "rent_free_months",
        "Notes /\nObservations":        "notes",
        "Market Rent\nEUR/m2/mo":       "market_rent_per_sqm_mo",
    }
    ...
    for col in ["area_sqm", "rent_per_sqm_mo", "annual_rent",
                "security_deposit", "rent_free_months", "market_rent_per_sqm_mo"]:
    ...
    # Use the EARLIER of lease expiry or break option date for each active tenant.
    # Woolworth June-2026 break (0.2yr) must not be masked by a 2028 expiry (2.7yr).
    if "break_option" in df.columns:
        _break_unexpired = ((df["break_option"] - today).dt.days / 365.25).clip(lower=0)
        _has_break = df["break_option"].notna()
        df.loc[_has_break, "unexpired_yrs"] = df.loc[_has_break, "unexpired_yrs"].combine(
            _break_unexpired.loc[_has_break], min
        )
...
def compute_metrics(deal: dict, rr: pd.DataFrame,
                    ...
                    noi_margin_override: Optional[float] = None,
                    market_rent_override: Optional[float] = None) -> dict:
    """
    ...
    # ── Reversion Risk: Market Rent vs Contract Rent ─────────────────────────
    asset_mkt_rent = float(market_rent_override or deal.get("market_rent_per_sqm_mo") or 0)

    reversion_data_available = False
    anchor_reversion_pct     = 0.0
    anchor_over_rented       = False
    portfolio_reversion_pct  = 0.0
    reversion_rows           = []

    _has_per_tenant_mkt = (
        "market_rent_per_sqm_mo" in active.columns
        and active["market_rent_per_sqm_mo"].sum() > 0
    )
    if asset_mkt_rent > 0 or _has_per_tenant_mkt:
        reversion_data_available = True
        _a = active.copy()
        if "market_rent_per_sqm_mo" not in _a.columns:
            _a["market_rent_per_sqm_mo"] = 0.0
        _a["_mkt"] = _a["market_rent_per_sqm_mo"].copy()
        if asset_mkt_rent > 0:
            _a.loc[_a["_mkt"] == 0, "_mkt"] = asset_mkt_rent
        _has_mkt = _a["_mkt"] > 0
        _a["_rev_pct"] = 0.0
        _a.loc[_has_mkt, "_rev_pct"] = (
            (_a.loc[_has_mkt, "rent_per_sqm_mo"] - _a.loc[_has_mkt, "_mkt"])
            / _a.loc[_has_mkt, "_mkt"]
        )
        if gross_passing_rent > 0 and _has_mkt.any():
            portfolio_reversion_pct = float(
                (_a.loc[_has_mkt, "_rev_pct"] * _a.loc[_has_mkt, "annual_rent"]).sum()
                / gross_passing_rent
            )
        _anchors = _a[_a["is_anchor"] & _has_mkt]
        if len(_anchors) > 0:
            _anch_rent = float(_anchors["annual_rent"].sum())
            if _anch_rent > 0:
                anchor_reversion_pct = float(
                    (_anchors["_rev_pct"] * _anchors["annual_rent"]).sum() / _anch_rent
                )
            anchor_over_rented = anchor_reversion_pct > 0.15
        for _, row in _a[_has_mkt].iterrows():
            reversion_rows.append({
                "tenant":        row["tenant"],
                "is_anchor":     bool(row.get("is_anchor", False)),
                "contract_rent": float(row["rent_per_sqm_mo"]),
                "market_rent":   float(row["_mkt"]),
                "reversion_pct": float(row["_rev_pct"]),
                "annual_rent":   float(row["annual_rent"]),
                "income_share":  float(row["annual_rent"] / gross_passing_rent) if gross_passing_rent > 0 else 0.0,
            })
    ...
    return {
        ...
        # Reversion Risk
        "reversion_data_available": reversion_data_available,
        "anchor_reversion_pct":     anchor_reversion_pct,
        "anchor_over_rented":       anchor_over_rented,
        "portfolio_reversion_pct":  portfolio_reversion_pct,
...
def run_qa_audit(metrics: dict, deal: dict) -> list:
    ...
    # ── Reversion Risk checks (only when market rent data is provided) ────────
    if m.get("reversion_data_available"):
        anch_rev = m.get("anchor_reversion_pct", 0)
        port_rev = m.get("portfolio_reversion_pct", 0)

        # Anchor reversion: >15% over market = RED FAIL (changes verdict)
        add("Anchor Reversion Risk <= 15%",
            anch_rev <= 0.15,
            anch_rev <= 0.25,
            f"{anch_rev:+.1%} vs market",
            "<= 15% over market",
            "RED")

        # Portfolio reversion: >20% weighted over-rent = RED FAIL
        add("Portfolio Reversion Risk <= 20%",
            port_rev <= 0.20,
            port_rev <= 0.30,
            f"{port_rev:+.1%} vs market",
            "<= 20% over market",
            "RED")
...
def generate_verdict(metrics: dict, qa: list) -> dict:
    ...
    # Hard DECLINE: WAULT below 2-year floor means no bank will finance at standard terms.
    # No point in DD spend until a lease extension is secured.
    if m.get("wault", 0) < 2.0:
        rec = "DECLINE"
        rat = (f"WAULT of {m['wault']:.1f} yrs is below the 2-year absolute floor. "
               "No German bank will provide senior debt. Secure lease extensions before any DD spend.")
    elif len(red_fails) >= 2:
